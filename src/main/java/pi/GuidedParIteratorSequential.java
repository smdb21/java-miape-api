...
public class GuidedParIteratorSequential<E> extends ParIteratorAbstract<E> {
	...
	public GuidedParIteratorSequential(Collection collection, int chunksize,
	...
	public GuidedParIteratorSequential(Iterator<E> it, int collectionSize,
			int chunksize, int numOfThreads, boolean ignoreBarrier) {
		super(numOfThreads, ignoreBarrier);
	...
	public boolean hasNext() {
		...
		if ((bufferIndex.get() >= 0)
				& (bufferIndex.get() < localChunksize.get())) {
			...
			if (it.hasNext()) {
				...
				localChunksize.set((int) Math.ceil(((double) remainingElements)
						/ numOfThreads));
