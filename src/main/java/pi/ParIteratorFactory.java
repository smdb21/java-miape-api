...
public class ParIteratorFactory<E> {
	 ...
	 * 
	 * @param iterator
	 *            The iterator for the collection that contains the elements to
	 *            be shared amongst the threads.
	 ...
	 * @return An instance of <code>ParIterator</code>.
	 */
	public static <E> ParIterator<E> createParIterator(Iterator<E> iterator,
			int collectionSize, int threadCount,
			ParIterator.Schedule schedulePol, int chunksize) {
		return getIterator(iterator, collectionSize, schedulePol, chunksize,
				threadCount, false);
	}
	...
	public static <E> ParIterator<E> createParIterator(Iterator<E> iterator,
			int collectionSize, int threadCount) {
		return getIterator(iterator, collectionSize,
				ParIterator.Schedule.DYNAMIC, ParIterator.DEFAULT_CHUNKSIZE,
				threadCount, false);
	}
	...
	public static <E> ParIterator<E> createParIterator(Iterator<E> iterator,
			int collectionSize, int threadCount,
			ParIterator.Schedule schedulePol) {
		return getIterator(iterator, collectionSize, schedulePol,
				ParIterator.DEFAULT_CHUNKSIZE, threadCount, false);
	}
	...
	private static <E> ParIterator<E> getIterator(Collection<E> collection,
		...
		case STATIC:
			if (collection instanceof RandomAccess) {
				return new StaticParIterator<E>((java.util.List<E>) collection,
						chunksize, threadCount, ignoreBarrier);
			} else {
				return new StaticParIterator<E>((E[]) collection.toArray(),
						chunksize, threadCount, ignoreBarrier);
			}
		case GUIDED:
			if (collection instanceof RandomAccess) {
				return new GuidedParIteratorRandomAccess<E>(
						(java.util.List<E>) collection, chunksize, threadCount,
						ignoreBarrier);
			} else {
				return new GuidedParIteratorSequential<E>(collection,
						chunksize, threadCount, ignoreBarrier);
			}
		case DYNAMIC:
			if (collection instanceof RandomAccess) {
				return new DynamicParIteratorRandomAccess<E>(
						(java.util.List<E>) collection, chunksize, threadCount,
						ignoreBarrier);
			} else {
				return new DynamicParIteratorSequential<E>(collection,
						chunksize, threadCount, ignoreBarrier);
			}
		default:
	...
	private static <E> ParIterator<E> getIterator(Iterator<E> iterator,
			int collectionSize, ParIterator.Schedule schedulePol,
			int chunksize, int threadCount, boolean ignoreBarrier) {
		switch (schedulePol) {
		case STATIC:

			throw new IllegalMiapeArgumentException(
					"STATIC schedule is not available for iterators");

		case GUIDED:

			return new GuidedParIteratorSequential<E>(iterator, collectionSize,
					chunksize, threadCount, ignoreBarrier);

		case DYNAMIC:

			return new DynamicParIteratorSequential<E>(iterator,
					collectionSize, chunksize, threadCount, ignoreBarrier);
