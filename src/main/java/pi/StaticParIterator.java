...
public class StaticParIterator<E> extends ParIteratorAbstract<E> {
	...
	public StaticParIterator(E[] collection, int chunksize, int numOfThreads,
	...
	public StaticParIterator(List<E> list, int chunksize, int numOfThreads,
			boolean ignoreBarrier) {
		super(numOfThreads, ignoreBarrier);
		// isFromList = true;
		this.chunksize = chunksize;
	...
	/*
	 * This method is called once by the constructor. It allocates each thread
	 * with the number of iterations it has to do.
	 */
	private void initialise() {

		int remainingElements = collectionSize;
	...
	public boolean hasNext() {
		...
		// -- this marks the beginning of the "iteration boundary" (for
		// exception handling). Therefore need to record the current element.
		...
		if (blocksDone[id] < blocksToDo[id]) {
			currentIndex[id] = currentIndex[id] + (numOfThreads - 1)
					* chunksize;
		...
		if (!successfullyCalledHasNext.get())
			throw new RuntimeException(
					"Thread "
							...
							+ " is calling next() without an associated call to hasNext()");
