...
public class MiapeMSIDocumentImpl implements MiapeMSIDocument {
	...
	private void processMzIdentMLInParallel() {
		...
		for (PSIPIAnalysisSearchSpectrumIdentificationType spectrumIdent : mzIdentML.getAnalysisCollection()
			...
			PSIPIAnalysisProcessProteinDetectionProtocolType proteinDetectionProtocol = getProteinDetectionProtocol(
			...
			Software msiSoftware = new SoftwareImpl(softwareXML, mzIdentML.getAuditCollection().getContactGroup(),
			...
			InputParameter inputParameter = new InputParameterImpl(spectrumIdentProtocol, proteinDetectionProtocol,
					databaseListXML, msiSoftware, inputParamID, numSeqSearched, cvManager);
		...
		if (mzIdentML.getAnalysisSoftwareList() != null) {
			Set<String> referencesSoftwareRefs = getProtocolsBySoftwareIDs(mzIdentML.getAnalysisProtocolCollection());
			for (PSIPIAnalysisSearchAnalysisSoftwareType analysisSoftware : mzIdentML.getAnalysisSoftwareList()
					.getAnalysisSoftware()) {
		...
		if (existsSourceFile()) {
			...
			PSIPIAnalysisSearchSourceFileType file = mzIdentML.getDataCollection().getInputs().getSourceFile().get(0);
	...
	private void processMzIdentML() {
		...
		for (PSIPIAnalysisSearchSpectrumIdentificationType spectrumIdent : mzIdentML.getAnalysisCollection()
			...
			for (PSIPIAnalysisSearchSpectrumIdentificationResultType spectIdentResultXML : spectrumIdentificationResult) {
				int percentage = count * 100 / spectrumIdentificationResult.size();
				...
				// new input Data
				PSIPISpectraSpectraDataType spectraDataXML = getSpectraData(spectIdentResultXML.getSpectraDataRef(),
				...
				if (!inputDataHash.containsKey(spectraDataXML.getId())) {
					...
					InputData inputData = new InputDataImpl(spectraDataXML, inputDataID);
				...
				for (PSIPIAnalysisSearchSpectrumIdentificationItemType spectIdentItemXML : spectrumIdentificationItems) {
					...
					PSIPIPolypeptidePeptideType peptideXML = getPeptide(spectIdentItemXML.getPeptideRef(),
	...
	private String getRetentionTimeInSeconds(PSIPIAnalysisSearchSpectrumIdentificationResultType spectIdentResultXML) {
		if (spectIdentResultXML != null) {
			if (spectIdentResultXML.getParamGroup() != null) {
				for (FuGECommonOntologyParamType paramType : spectIdentResultXML.getParamGroup()) {
	...
	private PSIPIAnalysisProcessProteinDetectionProtocolType getProteinDetectionProtocol(
		...
		if (pd != null && pd.getInputSpectrumIdentifications() != null) {
			for (InputSpectrumIdentificationsType isi : pd.getInputSpectrumIdentifications()) {
	...
	public String getGeneratedFilesDescription() {
		...
		int counter = 1;
		for (PSIPIAnalysisSearchSourceFileType file : mzIdentML.getDataCollection().getInputs().getSourceFile()) {
	...
	public Set<MSIAdditionalInformation> getAdditionalInformations() {
		...
		if (mzIdentML.getAnalysisSampleCollection() != null) {
			for (SampleType sampleXML : mzIdentML.getAnalysisSampleCollection().getSample()) {
