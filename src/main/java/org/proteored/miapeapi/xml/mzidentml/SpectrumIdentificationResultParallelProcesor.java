...
public class SpectrumIdentificationResultParallelProcesor extends Thread {
	...
	public SpectrumIdentificationResultParallelProcesor(
			ParIterator<PSIPIAnalysisSearchSpectrumIdentificationResultType> iterator, int numCore,
			ControlVocabularyManager cvManager, PSIPIMainMzIdentMLType mzIdentML,
			MapSync<String, InputData> syncInputDataHash, Reducible<List<IdentifiedPeptide>> reduciblePeptides,
	...
	public void run() {
		...
		while (iterator.hasNext()) {
			try {
				final PSIPIAnalysisSearchSpectrumIdentificationResultType spectIdentResultXML = iterator.next();
				...
				InputData inputData = syncInputDataHash.get(spectraDataXML.getId());
		...
		log.debug("Thread " + numThread + " processed " + peptides.size() + " peptides and " + proteinHash.size()
				+ " proteins");
	}
