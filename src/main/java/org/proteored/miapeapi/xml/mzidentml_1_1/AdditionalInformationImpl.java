...
public class AdditionalInformationImpl implements MSIAdditionalInformation {
	...
	public AdditionalInformationImpl(Sample sampleXML) {
		if (sampleXML != null) {
			...
			if (sampleXML.getParamGroup() != null) {
				value = MzidentmlControlVocabularyXmlFactory.readEntireParamList(sampleXML
						.getParamGroup());
			}
		}
	}

	public AdditionalInformationImpl(BibliographicReference referenceXML) {
		StringBuilder sb = new StringBuilder();
		if (referenceXML != null) {
			name = "Bibliographic reference";
			if (referenceXML.getAuthors() != null) {
				if (!referenceXML.getAuthors().equals("")) {
					sb.append(Utils.AUTHORS + "=" + referenceXML.getAuthors()
							+ MiapeXmlUtil.TERM_SEPARATOR);
				}
			}
			if (referenceXML.getEditor() != null) {
				if (!referenceXML.getEditor().equals("")) {
					sb.append(Utils.EDITOR + "=" + referenceXML.getEditor()
							+ MiapeXmlUtil.TERM_SEPARATOR);
				}
			}
