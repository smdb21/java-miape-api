...
public class IdentifiedProteinImplFromPepXMLProtein implements IdentifiedProtein {
	...
	public IdentifiedProteinImplFromPepXMLProtein(DTASelectProtein dtaSelectProtein,
		...
		if (peptides == null) {
			...
			if (psMs != null) {
				for (DTASelectPSM dtaSelectPSM : psMs) {
					if (IdentifiedPeptideImplFromPepXML.map.containsKey(dtaSelectPSM.getPsmIdentifier())) {
						final IdentifiedPeptide pep = IdentifiedPeptideImplFromPepXML.map
								.get(dtaSelectPSM.getPsmIdentifier());
						pep.getIdentifiedProteins().add(this);
						peptides.add(pep);
					} else {
						final IdentifiedPeptideImplFromPepXML pep = new IdentifiedPeptideImplFromPepXML(
								dtaSelectPSM, cvManager);
						IdentifiedPeptideImplFromPepXML.map.put(dtaSelectPSM.getPsmIdentifier(), pep);
						pep.getIdentifiedProteins().add(this);
						peptides.add(pep);
					}
