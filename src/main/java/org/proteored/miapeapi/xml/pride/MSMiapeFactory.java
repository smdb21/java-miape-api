...
public class MSMiapeFactory {
	...
	public MiapeMSDocument toDocument(MiapePrideXmlFile xmlFile,
			PersistenceManager databaseManager, ControlVocabularyManager cvManager, String user,
			String password, String projectName) throws MiapeDatabaseException,
		...
		try {
			...
			ExperimentCollection experimentCollection = (ExperimentCollection) jc
					.createUnmarshaller().unmarshal(xmlFile.toFile());
			log.info("PRIDE file unmarshalled");
			if (databaseManager == null) {
				result = new MiapeMSDocumentImpl(experimentCollection.getExperiment().get(0), null,
						xmlFile.toFile().getName(), projectName, cvManager);
			} else {
				result = new MiapeMSDocumentImpl(experimentCollection.getExperiment().get(0),
						databaseManager, cvManager, user, password, xmlFile.toFile().getName(),
						projectName);
			}

		} catch (UnmarshalException e) {
			e.printStackTrace();
			log.info("Input file format error. Check if the input file is PRIDE schema compliant here: http://www.ebi.ac.uk/pride/validateXML.do");
