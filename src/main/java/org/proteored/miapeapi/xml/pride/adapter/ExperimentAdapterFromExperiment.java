...
public class ExperimentAdapterFromExperiment implements Adapter<ExperimentType> {
	 ...
	 * @param addPeakList
	 */
	public ExperimentAdapterFromExperiment(ObjectFactory factory,
			ControlVocabularyManager cvManager, Experiment experiment,
			boolean addPeakList, boolean excludeNonConclusiveProteinGroups) {
		this(factory, cvManager, false, experiment, addPeakList, false,
	...
	public ExperimentAdapterFromExperiment(ObjectFactory factory,
			ControlVocabularyManager cvManager, boolean gelBasedExperiment,
			Experiment experiment, boolean addPeakList,
			boolean removeNoMatchedSpectra,
	...
	public ExperimentAdapterFromExperiment(ObjectFactory factory,
			...
			boolean excludeNonConclusiveProteinGroups) {
		this(factory, cvManager, false, experiment, addPeakList,
	...
	public ExperimentType adapt() {
		...
		xmlExperiment.setProtocol(new ProtocolAdapterFromExperiment(factory,
				cvManager, experiment).adapt());

		xmlExperiment.setMzData(new MzDataAdapter(factory, cvManager,
				MSResultingDataUtil.getMiapeMSsFromExperiment(experiment),
			...
			if (spectrumList != null) {
				...
				if (spectra != null) {
					...
					if (gelFreeIdentifications != null) {
						Set<Integer> spectrumRefs = new HashSet<Integer>();
						log.info("Removing not matched spectra from "
								+ numSpectra + " spectra");
						for (GelFreeIdentificationType gelFreeIdentification : gelFreeIdentifications) {
							final List<Peptide> peptideItems = gelFreeIdentification
									.getPeptideItem();
							for (Peptide peptide : peptideItems) {
								if (!spectrumRefs.contains(peptide
										.getSpectrumReference())) {
									spectrumRefs.add(Integer.valueOf(String
											.valueOf(peptide
													.getSpectrumReference())));
								}
							}
						}
						log.info(spectrumRefs.size()
								+ " spectra in the inclusion list");
			...
			if (spectrumList != null) {
				...
				if (spectra != null) {
					...
					if (gelFreeIdentifications != null) {
						...
						InnerIteratorSync2<Spectrum> iteratorSync = new InnerIteratorSync2(
								iterator, spectrumRefs);
						Collection<SpectraFilter> runners = new ArrayList<SpectraFilter>();
						int processorCount = SystemCoreManager
								.getAvailableNumSystemCores();
						...
						for (int i = 0; i < processorCount; i++) {
							...
							SpectraFilter runner = new SpectraFilter(
									iteratorSync, lock, i, spectrumRefs);
							runners.add(runner);
						...
						log.info(total
								+ " proteins processed successfully in parallel in "
								+ (System.currentTimeMillis() - t1) / 1000
								+ " sg.");
	...
	private void addGelFreeIdentifications(ExperimentType xmlExperiment) {
		if (this.experiment != null) {
			...
			SorterUtil
					.sortProteinGroupOcurrencesByProteinAccessionString(proteinGroupOccurrencelist);
			...
			for (ProteinGroupOccurrence proteinGroupOccurrence : proteinGroupOccurrencelist) {
				if ((this.excludeNonConclusiveProteinGroups && proteinGroupOccurrence
				...
				final List<ExtendedIdentifiedProtein> proteins = proteinGroupOccurrence
						.getProteins();
				HashMap<String, ProteinOccurrence> proteinOccurrences = DataManager
						.createProteinOccurrenceList(proteins);

				for (ProteinOccurrence proteinOccurrence : proteinOccurrences
						.values()) {
					xmlExperiment
							.getGelFreeIdentification()
							.add(new GelFreeIdentificationAdapterFromProteinOccurrence(
									factory,
									cvManager,
									proteinOccurrence,
									proteinGroupOccurrence.getProteinLocalFDR(),
									this.addPeakList).adapt());
				}
	...
	private void addAdditionals(ExperimentType xmlExperiment) {
		...
		if (this.experiment != null
				&& this.experiment.getPreviousLevelIdentificationSet() != null) {
			projectName = this.experiment.getPreviousLevelIdentificationSet()
					.getFullName();
		}
		...
		final ControlVocabularyTerm prideXmlSoftwareGenerator = AdditionalInformationName
				.getInstance(cvManager)
				.getCVTermByAccession(
						AdditionalInformationName.PRIDE_GENERATION_SOFTWARE_ACCESSION);
		CvParamType cvParamSoftware = prideCvUtil.createCvParam(
				prideXmlSoftwareGenerator.getPreferredName(),
				this.prideGenerationSoftware,
				AdditionalInformationName.getInstance(cvManager));
		...
		if (experiment != null) {
			...
			if (experiment.getDifferentPeptideModificationNames() == null
					|| experiment.getDifferentPeptideModificationNames()
							.isEmpty()) {
				final ControlVocabularyTerm noPTMsTerm = AdditionalInformationName
						.getInstance(cvManager).getCVTermByAccession(
								AdditionalInformationName.NO_PTMS);
				if (noPTMsTerm != null) {
					CvParamType cvParamNoPTms = prideCvUtil.createCvParam(
							noPTMsTerm.getPreferredName(), null,
							AdditionalInformationName.getInstance(cvManager));
			...
			if (this.gelBasedExperiment) {
				...
				CvParamType cvParamGelBasedExperiment = prideCvUtil
						.createCvParam(gelBasedExperimentTerm
			...
			final List<ResultingData> resultingDatas = experiment
					.getResultingDatas();
