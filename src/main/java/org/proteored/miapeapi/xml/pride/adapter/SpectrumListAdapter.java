...
public class SpectrumListAdapter implements Adapter<SpectrumList> {
	...
	private static final TIntObjectHashMap<Integer> miapeIDMGFSpectraOffsets = new TIntObjectHashMap<Integer>();
	 ...
	 */
	private static TObjectIntHashMap<String> mgfTitlesMaps = new TObjectIntHashMap<String>();

	private static Map<BigInteger, Spectrum> spectrumMap = new THashMap<BigInteger, Spectrum>();
	private static TDoubleObjectHashMap<Spectrum> spectrumByMoverZMap = new TDoubleObjectHashMap<Spectrum>();
	 ...
	 */
	public SpectrumListAdapter(ObjectFactory factory, ControlVocabularyManager cvManager,
			List<MiapeMSDocument> miapeMSs, boolean addPeakList) {
	 ...
	 * @return
	 */
	public static TIntObjectHashMap<Integer> getMiapeIDSpectraOffsets() {
	...
	public static Map<BigInteger, Spectrum> getSpectrumMap() {
		return SpectrumListAdapter.spectrumMap;
	}

	public static TDoubleObjectHashMap<Spectrum> getSpectrumByMoverZMap() {
		return SpectrumListAdapter.spectrumByMoverZMap;
	}

	@Override
	public SpectrumList adapt() {
		final SpectrumList spectrumList = factory.createExperimentTypeMzDataSpectrumList();
		if (miapeMSs != null) {
			...
			for (final MiapeMSDocument miapeMS : miapeMSs) {
				...
				if (addPeakList) {
					try {
						final boolean hasMGF = MSResultingDataUtil.hasMGFFile(miapeMS, cvManager);
						final boolean hasMzML = MSResultingDataUtil.hasMzMLFile(miapeMS, cvManager);
						final SpectraReader spectraReader = SpectraReader.getInstance(cvManager);
						if (hasMGF) {

							log.info("Parse spectra from MGF");
							final TMsData tMsData = spectraReader.getSpectraFromMGFs(
									MSResultingDataUtil.getMGFResultingDataFromMIAPEMS(miapeMS, cvManager),
									SpectrumListAdapter.mgfTitlesMaps);
							final TIntObjectHashMap<TSpectrum> spectra = tMsData.getSpectra();
							log.info(spectra.size() + " spectra added");

							// sort the queries numbers before to add to the
							// spectrum list
							final TIntArrayList queryList = new TIntArrayList();
							for (final int queryNumber : spectra.keys()) {
								queryList.add(queryNumber);
							}
							queryList.sort();
							;
							for (final int queryNumber : queryList.toArray()) {
								final TSpectrum tSpectrum = spectra.get(queryNumber);
								log.debug(queryNumber + " spectrum mass=" + tSpectrum.getPeptideMoverZ());
								Spectrum xmlSpectrum = null;
								try {
									xmlSpectrum = new SpectumAdapterFromMGFSpectrum(tMsData, queryNumber, offset,
											factory, cvManager).adapt();
								} catch (final IllegalMiapeArgumentException e) {
									log.warn(e.getMessage());
									continue;
								}
								SpectrumListAdapter.spectrumMap.put(
										BigInteger.valueOf(Long.valueOf(String.valueOf(xmlSpectrum.getId()))),
										xmlSpectrum);
								SpectrumListAdapter.spectrumByMoverZMap.put(tSpectrum.getPeptideMoverZ(), xmlSpectrum);
								spectrumList.getSpectrum().add(xmlSpectrum);
							}
							miapeIDMGFSpectraOffsets.put(miapeMS.getId(), offset);
						...
						if (hasMzML) {
							...
							numAddedSpectra = spectraReader.addSpectraFromMzML(spectrumList,
									MSResultingDataUtil.getMzMLResultingDataFromMIAPEMS(miapeMS, cvManager), offset,
									mzMLSpectrumIdentifierManager, miapeMS.getId());
	 ...
	 * @return
	 */
	public static BigInteger getPeptideSpectrumReference(Integer miapeMSReference, String spectrumRefOriginal) {
		try {
			final String key = miapeMSReference + spectrumRefOriginal;
			try {
				Integer.valueOf(spectrumRefOriginal);
			} catch (final NumberFormatException e) {
				// log.info("The spectrum ref is an string!");
				int offset = 0;
				if (SpectrumListAdapter.miapeIDMGFSpectraOffsets.containsKey(miapeMSReference)) {
					offset = SpectrumListAdapter.miapeIDMGFSpectraOffsets.get(miapeMSReference);
				}

				if (SpectrumListAdapter.mgfTitlesMaps.containsKey(spectrumRefOriginal)) {
					final Long l = Long.valueOf(SpectrumListAdapter.mgfTitlesMaps.get(spectrumRefOriginal) + offset);
					return BigInteger.valueOf(l);
				} else {
					// try to avoid return carriages in the spectrumRef.
					// Sometimes found in XTandemFiles
					if (spectrumRefOriginal.contains("\n")) {
						final String cutSpectrumRef = spectrumRefOriginal.substring(0,
								spectrumRefOriginal.indexOf("\n"));
						if (SpectrumListAdapter.mgfTitlesMaps.containsKey(cutSpectrumRef)) {
							final Long l = Long.valueOf(SpectrumListAdapter.mgfTitlesMaps.get(cutSpectrumRef) + offset);
							return BigInteger.valueOf(l);
						}
					}
				}
			...
			if (SpectrumListAdapter.mzMLSpectrumIdentifierManager.getSpectrumIndex(key) != -1)
				return BigInteger.valueOf(SpectrumListAdapter.mzMLSpectrumIdentifierManager.getSpectrumIndex(key));
			...
			if (SpectrumListAdapter.miapeIDMGFSpectraOffsets.containsKey(miapeMSReference)) {
				final int offset = SpectrumListAdapter.miapeIDMGFSpectraOffsets.get(miapeMSReference);
