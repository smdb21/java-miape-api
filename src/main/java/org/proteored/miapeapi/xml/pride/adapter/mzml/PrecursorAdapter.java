...
public class PrecursorAdapter implements Adapter<PrecursorType> {
	...
	public PrecursorAdapter(uk.ac.ebi.jmzml.model.mzml.Precursor mzMLPrecursor,
			ReferenceableParamGroupList referenceableParamGroupList, ObjectFactory factory,
			PrideControlVocabularyXmlFactory prideCvUtil, Spectrum precursorSpectrum, int offset) {
	...
	public PrecursorType adapt() {
		if (mzMLPrecursor != null) {
			...
			final SelectedIonList selectedIonList = mzMLPrecursor.getSelectedIonList();
			if (selectedIonList != null) {
				final List<ParamGroup> selectedIon = selectedIonList.getSelectedIon();
				if (selectedIon != null) {
			...
			if (mzMLactivation != null) {
				ParamType xmlActivation = prideCvUtil.getParamTypeFromMZMLParamGroup(
						mzMLactivation, referenceableParamGroupList);
			...
			try {
				if (precursorSpectrum != null) {
					...
					final ParamGroup paramGroup = MzMLControlVocabularyXmlFactory.createParamGroup(
							precursorSpectrum.getCvParam(), null,
							precursorSpectrum.getReferenceableParamGroupRef());

					final CVParam cv = MzMLControlVocabularyXmlFactory.getCvFromParamGroup(
							paramGroup, referenceableParamGroupList, MS_LEVEL_ACC);
					if (cv != null)
						prideXmlPrecursor.setMsLevel(Integer.valueOf(cv.getValue()));
					else
			...
			// spectrumRef comes from scan number in spectrum
			if (precursorSpectrum != null)
				prideXmlPrecursor.setSpectrumRef(precursorSpectrum.getIndex() + 1 + offset);
			else
	...
	private ParamType getSelectedIon(ParamGroup selectedIonParamGroup) {
		...
		// charge state
		String chargeStateValue = MzMLControlVocabularyXmlFactory.getValueFromParamGroup(
				selectedIonParamGroup, referenceableParamGroupList,
				ChargeState.getInstance(cvManager));
		if (chargeStateValue == null) {
			log.info("No charge state value for the spectrum. Assuming charge state=1");
			chargeStateValue = "1";
		}
		ControlVocabularyTerm chargeStateTerm = ChargeState.getInstance(cvManager)
				.getChargeStateTerm();
		prideCvUtil.addCvParamToParamType(prideXmlIonSection, chargeStateTerm.getTermAccession(),
				chargeStateTerm.getPreferredName(), chargeStateValue, chargeStateTerm.getCVRef());

		// m/z
		String mzOverz = MzMLControlVocabularyXmlFactory.getValueFromParamGroup(
				selectedIonParamGroup, referenceableParamGroupList, MOverZ.getInstance(cvManager));
		if (mzOverz != null) {
			ControlVocabularyTerm mOverZTerm = MOverZ.getMOverZTerm(cvManager);
			prideCvUtil.addCvParamToParamType(prideXmlIonSection, mOverZTerm.getTermAccession(),
					mOverZTerm.getPreferredName(), mzOverz, mOverZTerm.getCVRef());

		} else {
			log.info("No m/z value for the spectrum");
		}
