...
public class GelFreeIdentificationAdapterFromProteinOccurrence implements Adapter<GelFreeIdentificationType> {
	...
	public GelFreeIdentificationType adapt() {
		...
		List<Database> proteinDatabases = proteinOccurrence.getProteinDatabases();
	...
	private void addPeptides(GelFreeIdentificationType xmlProtein, List<ExtendedIdentifiedPeptide> peptideList) {
		...
		for (ExtendedIdentifiedPeptide identifiedPeptide : peptideList) {
			if (!peptideIds.contains(identifiedPeptide.getId())) {
				...
				try {
					Peptide xmlPeptide = new PeptideAdapter(factory, cvManager, identifiedPeptide, this.includeSpectra)
							.adapt();
	...
	private void addAdditionalParameters(GelFreeIdentificationType gelFreeIdentification) {
		try {
			...
			ExtendedIdentifiedProtein protein = proteinOccurrence.getFirstOccurrence();
			...
			if (scoreNames != null)
				for (String scoreName : scoreNames) {
					List<Float> scoreValues = proteinOccurrence.getScoreValues(scoreName);
