...
public class IdentifiedPeptideImpl implements IdentifiedPeptide {
	private static final Accession PRIDE_RANK_CV = new Accession("PRIDE:0000091");
	...
	public IdentifiedPeptideImpl(Peptide peptide, Set<InputDataSet> inputDataSets, Integer identifier,
	...
	public String getCharge() {
		if (peptidePRIDE.getAdditional() != null) {
			final CvParamType cvParam = PrideControlVocabularyXmlFactory
					.getCvFromParamType(peptidePRIDE.getAdditional(), ChargeState.CHARGE_STATE_ACCESSION);
			if (cvParam != null)
	...
	public Set<PeptideModification> getModifications() {
		Set<PeptideModification> pepMods = new THashSet<PeptideModification>();
		List<Modification> modificationItemList = peptidePRIDE.getModificationItem();
		if (modificationItemList != null) {
			for (Modification modificationItem : modificationItemList) {
				pepMods.add(new PeptideModificationImpl(modificationItem, peptidePRIDE.getSequence(), cvManager));
			}
		}
		if (pepMods.size() > 0)
			return pepMods;
	...
	public Set<PeptideScore> getScores() {
		...
		if (additional == null)
			return null;
		Set<PeptideScore> scores = new THashSet<PeptideScore>();
		for (Object param : additional.getCvParamOrUserParam()) {
			if (param instanceof CvParamType) {
				CvParamType cvParam = (CvParamType) param;
				if (Score.getInstance(cvManager).getCVTermByPreferredName(cvParam.getName()) != null)
				...
				if (PrideControlVocabularyXmlFactory.findTextInUserParamType(userParam, SCORE_TEXT_LIST,
						MatchMode.ANYWHERE)) {
	...
	public int getRank() {
		if (peptidePRIDE.getAdditional() != null) {
			final CvParamType cvParam = PrideControlVocabularyXmlFactory
					.getCvFromParamType(peptidePRIDE.getAdditional(), PRIDE_RANK_CV);
			if (cvParam != null) {
				try {
					...
					if (object instanceof CvParamType) {
						...
						if (cvParam.getValue() != null) {
							try {
								...
								if (cvParam.getAccession().equals("PRIDE:0000203"))
