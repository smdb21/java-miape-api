...
import org.proteored.miapeapi.xml.pride.util.PridePSMIdentifier;
import org.proteored.miapeapi.xml.util.MiapeXmlUtil;
import org.proteored.miapeapi.xml.util.PeptideCacheBySpectrumTitle;
	...
	public List<IdentifiedPeptide> getIdentifiedPeptides() {
		if (peptides == null) {
			...
			for (Peptide peptide : protein.getPeptideItem()) {
				IdentifiedPeptide newPeptide = null;
				String psmID = PridePSMIdentifier.getPSMUniqueIdentifier(peptide);

				if (psmID != null && peptidesBySpectrumTitle.contains(psmID)) {
					newPeptide = peptidesBySpectrumTitle.getFromCache(psmID);
					// add this protein
					if (newPeptide instanceof IdentifiedPeptideImpl) {
						final List<IdentifiedProtein> identifiedProteins = ((IdentifiedPeptideImpl) newPeptide)
								.getIdentifiedProteins();
						if (!identifiedProteins.contains(this)) {
							identifiedProteins.add(this);
						}
					}
				}

				if (newPeptide == null) {
					newPeptide = new IdentifiedPeptideImpl(peptide, inputDataSets,
							MiapeXmlUtil.PeptideCounter.increaseCounter(), cvManager, this);
					// add to static cache
					if (psmID != null) {
						peptidesBySpectrumTitle.addtoCache(newPeptide, psmID);
					}
				}
				peptides.add(newPeptide);
