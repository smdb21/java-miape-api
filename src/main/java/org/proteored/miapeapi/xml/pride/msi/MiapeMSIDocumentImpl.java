...
public class MiapeMSIDocumentImpl extends AbstractDocumentFromPride implements MiapeMSIDocument {
	...
	private void processPRIDEFile(ExperimentType experiment) {
		...
		// InputDataSet
		// One inputDataSet
		inputDataSets.add(new InputDataSetImpl(experiment.getMzData(), MiapeIdentifierCounter.increaseCounter()));
		...
		final InputParameterImpl inputParameter = new InputParameterImpl(experiment, MSISoftwares,
				MiapeIdentifierCounter.increaseCounter(), cvManager);
	...
	private Set<Software> getMSISoftwares(ExperimentType experiment) {
		final Set<Software> result = new THashSet<Software>();
		final Set<String> softwares = new THashSet<String>();
		for (final Object object : experiment.getGelFreeIdentification()) {
			final GelFreeIdentificationType identification = (GelFreeIdentificationType) object;
		...
		for (final Object object : experiment.getTwoDimensionalIdentification()) {
			final TwoDimensionalIdentificationType identification = (TwoDimensionalIdentificationType) object;
			if (identification.getSearchEngine() != null
					&& softwares.contains(identification.getSearchEngine()) == false) {
				softwares.add(identification.getSearchEngine());
			}
		}
		for (final String software : softwares) {
			result.add(new SoftwareImpl(software, MiapeIdentifierCounter.increaseCounter()));
		}
		return result;
	}
	...
	public Set<MSIAdditionalInformation> getAdditionalInformations() {
		final Set<MSIAdditionalInformation> setOfAdditionalInformations = new THashSet<MSIAdditionalInformation>();
		...
		if (experiment.getProtocol() != null) {
			...
			if (experiment.getProtocol().getProtocolSteps() != null) {
				for (final ParamType paramType : experiment.getProtocol().getProtocolSteps().getStepDescription()) {
					final List<Parameter> listParameter = Utils.createListParameter(paramType.getCvParamOrUserParam(),
							null);
					if (listParameter != null)
						for (final Parameter param : listParameter) {
			...
			if (experiment.getMzData() != null && experiment.getMzData().getDescription() != null
				...
				if (sampleDescription != null && sampleDescription.getCvParamOrUserParam() != null) {
					for (final Object cvOrUserParam : sampleDescription.getCvParamOrUserParam()) {
	...
	public String getName() {
		...
		if (experiment.getAdditional() != null) {
			for (final Object cvOrUserParam : experiment.getAdditional().getCvParamOrUserParam()) {
				if (cvOrUserParam instanceof CvParamType) {
	...
	public List<IdentifiedPeptide> getIdentifiedPeptides() {
		...
		final Set<IdentifiedPeptide> set = new THashSet<IdentifiedPeptide>();
		final Set<IdentifiedProteinSet> identifiedProteinSets = getIdentifiedProteinSets();
		if (identifiedProteinSets != null) {
			for (final IdentifiedProteinSet proteinSet : identifiedProteinSets) {
				if (proteinSet.getIdentifiedProteins() != null) {
					for (final String proteinAcc : proteinSet.getIdentifiedProteins().keySet()) {
