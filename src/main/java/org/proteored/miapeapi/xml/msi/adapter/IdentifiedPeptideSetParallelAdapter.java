...
public class IdentifiedPeptideSetParallelAdapter implements Adapter<MSIIdentifiedPeptideSet> {
	...
	public IdentifiedPeptideSetParallelAdapter(List<IdentifiedPeptide> identifiedPeptides, ObjectFactory factory,
	...
	public MSIIdentifiedPeptideSet adapt() {
		...
		if (identifiedPeptides != null && !identifiedPeptides.isEmpty()) {
			...
			for (int numCore = 0; numCore < threadCount; numCore++) {
				...
				IdentifiedPeptideParallelProcessor runner = new IdentifiedPeptideParallelProcessor(numCore, iterator,
						reduciblePeptides, factory, cvFactory);
				runners.add(runner);
				runner.start();
			}
			...
			if (Thread.currentThread().isInterrupted()) {
				for (int k = 0; k < threadCount; k++) {
					runners.get(k).interrupt();
				}
				throw new InterruptedMIAPEThreadException("Data import cancelled");
			}

			final ParIteratorException<IdentifiedPeptide>[] allExceptions = iterator.getAllExceptions();
			if (allExceptions.length != 0) {
				throw new IllegalArgumentException(allExceptions[0].getException());
			}
			// Reductors
			Reduction<List<MSIIdentifiedPeptide>> peptideListReduction = new Reduction<List<MSIIdentifiedPeptide>>() {
				@Override
				public List<MSIIdentifiedPeptide> reduce(List<MSIIdentifiedPeptide> first,
