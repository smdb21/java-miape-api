...
public class IdentifiedPeptideImpl implements IdentifiedPeptide {
	...
	public IdentifiedPeptideImpl(MSIIdentifiedPeptide xmlIdentifiedPeptide,
		...
		if (this.id == -1) {
			...
			xmlIdentifiedPeptide.setId(MiapeXmlUtil.IdentifierPrefixes.PEPTIDE
					.getPrefix() + this.id);
		}
	...
	public Set<PeptideScore> getScores() {
		...
		final List<ParamType> msiPeptideScore = xmlIdentifiedPeptide
				.getPeptideScore();
	...
	public InputData getInputData() {
		if (mapInputData.containsKey(this.xmlIdentifiedPeptide
				.getInputDataRef())) {
			return new InputDataImpl(
					this.mapInputData.get(this.xmlIdentifiedPeptide
							.getInputDataRef()));
		}
		return null;
	}
	...
	public Set<PeptideModification> getModifications() {
		Set<PeptideModification> setOfPeptideModifications = new HashSet<PeptideModification>();
		List<MSIPeptideModification> xmlModifications = xmlIdentifiedPeptide
				.getMSIPeptideModification();
		if (xmlModifications != null) {
			for (MSIPeptideModification xmlModification : xmlModifications) {
				setOfPeptideModifications.add(new PeptideModificationImpl(
						xmlModification));
			}
