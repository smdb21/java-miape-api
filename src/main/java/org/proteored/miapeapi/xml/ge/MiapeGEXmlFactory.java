...
public class MiapeGEXmlFactory implements XmlMiapeFactory<MiapeGEDocument> {
	...
	@Override
	public MiapeGEDocument toDocument(MiapeXmlFile<MiapeGEDocument> xmlFile,
			ControlVocabularyManager cvManager, PersistenceManager db, String userName,
	...
	private MiapeGEDocument create(MiapeXmlFile<MiapeGEDocument> xmlFile,
			PersistenceManager databaseManager, ControlVocabularyManager cvManager,
			String userName, String password) {
		MiapeGEDocument result = null;
		try {

			GEMIAPEGE miapeGE = (GEMIAPEGE) jc.createUnmarshaller().unmarshal(xmlFile.toFile());
			if (databaseManager == null) {
				result = new MiapeGEDocumentImpl(miapeGE, null, cvManager, userName, password);
			} else {
				result = new MiapeGEDocumentImpl(miapeGE, cvManager, databaseManager, userName,
						password);
			}
		} catch (JAXBException e) {
			e.printStackTrace();
	...
	private MiapeXmlFile<MiapeGEDocument> create(MiapeGEDocument document,
		...
		try {
			...
			GEMIAPEGE miapeGE = new MiapeGeXMLAdapter(document, controlVocabularyUtil).adapt();
	...
	@Override
	public MiapeXmlFile<MiapeGEDocument> toXml(MiapeGEDocument document,
