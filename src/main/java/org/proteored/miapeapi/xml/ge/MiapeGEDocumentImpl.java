...
public class MiapeGEDocumentImpl implements MiapeGEDocument {
	...
	public MiapeGEDocumentImpl(GEMIAPEGE miapeGEXML, User owner, ControlVocabularyManager controlVocabularyUtil,
		...
		if (owner != null)
			...
			if (miapeGEXML != null && miapeGEXML.getMIAPEProject() != null)
				user = new UserImpl(miapeGEXML.getMIAPEProject().getOwnerName(), userName, password);
			else
				user = null;
		}
	...
	public MiapeGEDocumentImpl(GEMIAPEGE miapeGEXML, ControlVocabularyManager controlVocabularyUtil,
			PersistenceManager dbManager, String userName, String password)
			throws MiapeDatabaseException, MiapeSecurityException {
		this(miapeGEXML, dbManager.getUser(userName, password), controlVocabularyUtil, userName, password);
		this.dbManager = dbManager;
		cvUtil = controlVocabularyUtil;
	}

	private Map<String, GEGelMatrix> initGelMatrixMap() {
		Map<String, GEGelMatrix> map = new THashMap<String, GEGelMatrix>();

		List<GEProtocol> geProtocol = miapeGeXML.getGEProtocol();
		if (geProtocol != null) {
			for (GEProtocol protocolItem : geProtocol) {
				List<GEDimensionType> geFirstDimension = protocolItem.getGEFirstDimension();
				if (geFirstDimension != null) {
					for (GEDimensionType dimensionItem : geFirstDimension) {
						List<GEGelMatrix> geGelMatrix = dimensionItem.getGEGelMatrix();
						if (geGelMatrix != null) {
	...
	private Map<String, GESample> initMapLaneSample() {
		Map<String, GESample> map = new THashMap<String, GESample>();
	...
	public Set<DirectDetection> getDirectDetections() {
		...
		List<GEDirectDetection> directDetectionList = miapeGeXML.getGEDirectDetection();
		if (directDetectionList != null) {
			Set<DirectDetection> setOfDirectDetections = new THashSet<DirectDetection>();
			...
			for (GEDirectDetection directDetectionItem : directDetectionList) {
				setOfDirectDetections.add(new DirectDetectionImpl(directDetectionItem));
			}
	...
	public Set<ImageAcquisition> getImageAcquisitions() {
		Set<ImageAcquisition> setOfImageAcquisition = new THashSet<ImageAcquisition>();
		List<GEImageAcquisition> imageAcquisitionList = miapeGeXML.getGEImageAcquisition();
		if (imageAcquisitionList != null) {
			for (GEImageAcquisition imageAcquisitionItem : imageAcquisitionList)
				setOfImageAcquisition.add(new ImageAcquisitionImpl(imageAcquisitionItem, gelMatrixMap, sampleMap));
	...
	public Set<ImageGelElectrophoresis> getImages() {
		...
		if (imageList != null) {
			Set<ImageGelElectrophoresis> setOfImages = new THashSet<ImageGelElectrophoresis>();
			for (GEImage geImage : imageList) {
				setOfImages.add(new ImageImpl(geImage, gelMatrixMap, sampleMap));
			}
			return setOfImages;
		}
		return null;
	...
	public Set<IndirectDetection> getIndirectDetections() {
		...
		List<GEIndirectDetection> indirectDetectionList = miapeGeXML.getGEIndirectDetection();
		if (indirectDetectionList != null) {
			Set<IndirectDetection> setOfIndirectDetection = new THashSet<IndirectDetection>();

			for (GEIndirectDetection geIndirectDetection : indirectDetectionList) {
				setOfIndirectDetection.add(new IndirectDetectionImpl(geIndirectDetection));
			}
	...
	public Set<Protocol> getProtocols() {
		Set<Protocol> setOfProtocols = new THashSet<Protocol>();
	...
	public Set<Sample> getSamples() {
		Set<Sample> setOfSamples = new THashSet<Sample>();
	...
	public Date getModificationDate() {
		XMLGregorianCalendar modificationDate = miapeGeXML.getModificationDate();
	...
	public Project getProject() {
		if (id > 0 && dbManager != null) {
			try {
				return dbManager.getMiapeGEPersistenceManager().getMiapeById(id, user.getUserName(), user.getPassword())
						.getProject();
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void delete(String userName, String password) throws MiapeDatabaseException, MiapeSecurityException {
		if (dbManager != null && dbManager.getMiapeGEPersistenceManager() != null) {
			dbManager.getMiapeGEPersistenceManager().deleteById(getId(), userName, password);
		}
	...
	public Set<GEAdditionalInformation> getAdditionalInformations() {
		Set<GEAdditionalInformation> setOfAdditionalInformation = new THashSet<GEAdditionalInformation>();
		List<ParamType> additionalInformationList = miapeGeXML.getGEAdditionalInformation();
		if (additionalInformationList != null) {
			for (ParamType geAdditionalInformation : additionalInformationList) {
				setOfAdditionalInformation.add(new AdditionalInformationImpl(geAdditionalInformation));
			}
