...
public class EsiImpl implements Esi {
	...
	private final Set<Equipment> interfaces = new THashSet<Equipment>();
	private final Set<Equipment> sprayers = new THashSet<Equipment>();
	private final ControlVocabularyManager cvManager;

	public EsiImpl(SourceComponent sourceComponent, ReferenceableParamGroupList referenceableParamGroupList,
		...
		if (sourceComponent != null) {
			...
			if (this.isESIFromParamGroup(paramGroup, referenceableParamGroupList)) {
				...
				this.supplyType = MzMLControlVocabularyXmlFactory.getFullCVFromParamGroupByName(paramGroup,
						referenceableParamGroupList, SUPPLY_TYPE_TEXT_LIST, MatchMode.ANYWHERE);
	...
	private void processInterfaces(ParamGroup paramGroup, ReferenceableParamGroupList referenceableParamGroupList) {

		InterfaceImpl interfaze = new InterfaceImpl(paramGroup, referenceableParamGroupList);

		if (interfaze != null && interfaze.getName() != null && !"".equals(interfaze.getFullDescription()))
			interfaces.add(interfaze);

	}

	private void processSprayers(ParamGroup paramGroup, ReferenceableParamGroupList referenceableParamGroupList,
			ControlVocabularyManager cvManager) {
		SprayerImpl sprayer = new SprayerImpl(paramGroup, referenceableParamGroupList, cvManager);
		if (sprayer != null && sprayer.getName() != null && !"".equals(sprayer.getFullDescription()))
