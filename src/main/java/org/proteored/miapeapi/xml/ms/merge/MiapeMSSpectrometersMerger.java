...
public class MiapeMSSpectrometersMerger implements MiapeMerger<Set<Spectrometer>> {
	...
	@Override
	public Set<Spectrometer> merge(Set<Spectrometer> miapeDataOriginal,
			Set<Spectrometer> miapeDataMetadata) {
		...
		Set<Spectrometer> ret = new HashSet<Spectrometer>();
		...
		for (int i = 0; i < maxIndex; i++) {
			...
			Object object;
			String name = null;
			object = MergerUtil.getNonNullValue(spectrometerOriginal, spectrometerMetadata,
					"getName");
			if (object != null)
				name = (String) object;
			...
			int id = -1;
			object = MergerUtil
					.getNonNullValue(spectrometerOriginal, spectrometerMetadata, "getId");
			if (object != null)
				id = (Integer) object;
			final SpectrometerBuilder builder = (SpectrometerBuilder) MiapeMSDocumentFactory
					.createSpectrometerBuilder(name).customizations(customizations)
					.parameters(parameters).version(version).catalogNumber(catalogNumber)
					.comments(comments).description(description).manufacturer(manufacturer)
					.model(model).uri(uri).id(id);
